...
//         // --- Stripe MCP requires a secret key ---
//         if std::env::var("STRIPE_SECRET_KEY").is_err() {
//             println!("\nSTRIPE_SECRET_KEY not set, skipping test");
//             return;
//         }
//         // --- Filesystem MCP needs no env var (remove the block above when reverting) ---
...
//         // Built-in tools (read, write, glob, bash) go under Tools namespace
//         assert!(agent_view.system_prompt.contains("namespace Tools"));

//         assert!(agent_view.system_prompt.contains("namespace Filesystem"));
//     }
// }
