...
pub trait ModelProvider: Send + Sync {
    /// Complete a prompt with the model, returning action and token count
    async fn complete(
        &self,
        messages: &[Message],
        tools: &[Tool],
    ...
    fn calculate_cost(&self, input_tokens: u64, output_tokens: u64) -> Cost {
        let input_cost =
            Cost::from_microdollars(self.cost_per_input_token().microdollars() * input_tokens);
        let output_cost =
            Cost::from_microdollars(self.cost_per_output_token().microdollars() * output_tokens);
